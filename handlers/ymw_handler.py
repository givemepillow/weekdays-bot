...
async def days_of_week_in_month(message: types.Message, state: FSMContext):
    ...
    if message.text not in weekdays:
        ...
        async with state.proxy() as data:
            ...
            year = data['year']
            month_start, month_end = data['month'], data['month'] + 1
            if month_end > 12:
                month_end = 1
            if month_start == 12:
            ...
            count_of_days = np.busday_count(f"{year}-{month_start:02}",
                                            f"{end_year}-{month_end:02}",
                                            weekmask=f"{days[weekdays.index(message.text)]}")
            await message.answer(f"{message.text}:\nc {start_day:02}.{month_start:02}.{year} по "
                                 f"{end_day:02}.{month_start:02}.{year} (включительно)\n"
                                 f" {count_of_days} шт.",
